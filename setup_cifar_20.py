...
class CIFAR_20:
    def __init__(self):
        ...
        data_set_path = "../cifar_20/cifar-100-binary/"

        r, s1, s2 = load_batch(data_set_path + "train.bin")
        train_data.extend(r)
        train_labels.extend(s1)

        train_data = np.array(train_data, dtype=np.float32)
        train_labels = np.array(train_labels)

        self.test_data, self.test_labels, self.test_labels_f = load_batch_test(data_set_path + "test.bin")
