...
class cnn_EADEN_cov:
    def __init__(self, sess, model, batch_size=1, confidence = CONFIDENCE,
                 ...
                 boxmin = -0.5, boxmax = 0.5, beta = BETA):
        """
        ...
        self.output = tf.cast(model.predict(tf.cast(self.newimg,tf.float64)),tf.float32)

        # distance to the input data
        self.l2dist = tf.reduce_sum(tf.square(self.newimg - (tf.tanh(self.timg) * self.boxmul + self.boxplus)),
                                    [1, 2, 3])
        self.l1dist = tf.reduce_sum(tf.abs(self.newimg - (tf.tanh(self.timg) * self.boxmul + self.boxplus)),
                                    [1, 2, 3])
    ...
    def attack_batch(self, imgs, labs):
        ...
        for outer_step in range(self.BINARY_SEARCH_STEPS):
            ...
            for iteration in range(self.MAX_ITERATIONS):
                # perform the attack
                _, l, l2s, l1s, elastic, scores, nimg = self.sess.run([self.train, self.loss,
                                                         self.l2dist, self.l1dist, self.elasticdist, self.output,
                                                         self.newimg])
